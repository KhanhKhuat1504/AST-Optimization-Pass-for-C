...
void ASTExpressionBool2Int::MyOptznPass(std::unique_ptr<ASTExpression> &parentPtr, ASTFunction &func)
{
    if (operand)
...
llvm::Value *ASTExpressionBool2Int::Compile(llvm::IRBuilder<> &builder, ASTFunction &func)
{
    ...
    return builder.CreateSExt(operand->CompileRValue(builder, func), VarTypeSimple::IntType.GetLLVMType(builder.getContext()));
}
