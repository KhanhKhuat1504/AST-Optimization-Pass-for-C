...
void ASTExpressionCall::MyOptznPass(std::unique_ptr<ASTExpression> &parentPtr, ASTFunction &func)
{
    if (callee)
        callee->MyOptznPass(callee, func);
    for (std::unique_ptr<ASTExpression> &exprPtr : arguments)
        exprPtr->MyOptznPass(exprPtr, func);
}
