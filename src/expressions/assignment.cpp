...
void ASTExpressionAssignment::MyOptznPass(std::unique_ptr<ASTExpression> &parentPtr, ASTFunction &func)
{
    if (left)
        left->MyOptznPass(left, func);
    if (right)
        right->MyOptznPass(right, func);
}
...
llvm::Value *ASTExpressionAssignment::Compile(llvm::IRBuilder<> &builder, ASTFunction &func)
{
    ...
    if (!left->IsLValue(func))
        throw std::runtime_error("ERROR: Left side of assignment expression is not an L-Value!");
