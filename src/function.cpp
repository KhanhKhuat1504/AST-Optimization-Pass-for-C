...
void ASTFunction::Define(std::unique_ptr<ASTStatement> definition)
{
    if (!this->definition) // Define only if not already defined.
    {
        ...
        throw std::runtime_error("ERROR: Function " + name + " already has a definition!");
}

void ASTFunction::MyOptznPass()
{
    if (!definition)
        return;
    definition->MyOptznPass(definition, *this);
}
...
void ASTFunction::Compile(llvm::Module &mod, llvm::IRBuilder<> &builder)
{
    ...
    */
    ...
    if (!satisfiesType && retType)
        satisfiesType = retType->Equals(funcType->returnType.get()); // If we return something, make sure we return what is expected.
    if (!satisfiesType)
    {
