...
ASTFunction *AST::GetFunction(const std::string &name)
{
    ...
    if (found != functions.end())
        ...
        throw std::runtime_error("ERROR: Function " + name + " can not be found in the ast!");
}

void AST::MyOptznPass()
{
    for (auto &func : functionList)
    {
        std::cout << "INFO: Optimizing function " + func + "." << std::endl;
        functions[func]->MyOptznPass();
    }
}
...
{
    if (!compiled)
        throw std::runtime_error("ERROR: Module " + std::string(module.getName().data()) + " not compiled!");
    if (outFile == "")
        throw std::runtime_error("ERROR: Writing assembly to standard out is not supported!");
    std::error_code err;
...
void AST::WriteLLVMBitcodeToFile(const std::string &outFile)
{
    if (!compiled)
        throw std::runtime_error("ERROR: Module " + std::string(module.getName().data()) + " not compiled!");
    if (outFile == "")
        throw std::runtime_error("ERROR: Writing bitcode to standard out is not supported!");
    std::error_code err;
